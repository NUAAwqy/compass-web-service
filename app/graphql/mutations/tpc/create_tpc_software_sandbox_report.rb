...
module Mutations
  module Tpc
    class CreateTpcSoftwareSandboxReport < BaseMutation
      ...
      argument :software_report, Input::TpcSoftwareSandboxReportInput, required: true
      ...
      def resolve(label: nil,
        ...
        tpc_software_sandbox_report = TpcSoftwareSandboxReport.find_by(subject_id: subject.id, code_url: software_report.code_url)
        raise GraphQL::ExecutionError.new I18n.t('tpc.software_report_already_exist') if tpc_software_sandbox_report.present?
        raise GraphQL::ExecutionError.new I18n.t('tpc.software_code_url_invalid') unless TpcSoftwareSandboxReportMetric.check_url(software_report.code_url)
        ...
        ActiveRecord::Base.transaction do
          ...
          software_report_data["short_code"] = TpcSoftwareSandboxReport.generate_short_code
          report = TpcSoftwareSandboxReport.create(software_report_data)
          ...
          report_metric = report.tpc_software_sandbox_report_metrics.create!(
            {
          ...
          tpc_software_metric_server.analyze_metric_by_tpc_service(report.id, report_metric.id, report.oh_commit_sha, TpcSoftwareMetricServer::Report_Type_Sandbox)
          tpc_software_metric_server.analyze_metric_by_compass(report.id, report_metric.id, TpcSoftwareMetricServer::Report_Type_Sandbox)
        end
